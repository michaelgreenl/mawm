...
- `.mawm/initiatives/roadmap.md`, initiative docs under `active/`, and phase docs under `phases/active/` are the current planning source of truth.
...
- Active plans should mention older initiative or phase docs only when that reference is absolutely necessary to invalidate a stale assumption, explain a migration boundary, or point at historical implementation detail.

## Historical Records

- `archived/` initiatives are kept for history and logging only.
- `phases/complete/` initiative phase docs are kept for history and logging only.
...
- They are planning input, not authoritative over the roadmap, active plans, or the current codebase.
