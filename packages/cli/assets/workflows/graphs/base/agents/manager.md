...
Broader replanning stays user-controlled. If execution needs phase, initiative, or architecture replanning, surface that to the user and let them run `/replan`; do not use the hidden planner subagent for anything beyond approved-step handoffs.
...
8. Treat the current run scope as fixed by runtime nearest-ancestor `.mawm/` discovery. Do not switch to a different MAWM root mid-run.
9. Derive or confirm the target active step-plan path for the selected step under `steps/active/<nn>-<step-slug>/plan.md` and the sibling `log.md` path in the same directory.
...
- The target step `plan.md` path under `steps/active/<nn>-<step-slug>/plan.md`
...
- The exact path to the step `log.md` file beside the active step `plan.md`
...
- Never use the hidden planner subagent for broader replanning; surface the need and wait for the user to run `/replan`
