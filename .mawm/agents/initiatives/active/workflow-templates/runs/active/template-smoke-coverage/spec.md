...
- Each emitted template root must remain a self-contained scaffoldable workflow workspace containing the shared bundle surface: `package.json`, `langgraph.json`, `langgraph.dist.json`, `mawm.json`, `scripts/build.js`, `src/graph/index.ts`, and `test/`, plus the variant-owned files declared in `overlay.json`.
