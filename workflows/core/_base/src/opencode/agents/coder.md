...
- Path to the step `log.md` file beside the active step `plan.md`
...
Write the step log to the exact `log.md` path the manager provides.
...
The step log lives beside the active step `plan.md`. Do not invent a different filename or directory layout.
...
function a(value: unknown): void {
  if (isRecord(value)) {
    /* ... */
  }
}
function b(value: unknown): void {
  if (isRecord(value)) {
    /* ... */
  }
}
